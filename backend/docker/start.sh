...
MAX_TRIES=30
COUNT=0
until php -r "new PDO('mysql:host=${DB_HOST:-mysql};port=${DB_PORT:-3306};dbname=${DB_DATABASE:-railway}', '${DB_USERNAME:-root}', '${DB_PASSWORD:-}');" 2>/dev/null || [ $COUNT -eq $MAX_TRIES ]; do
  COUNT=$((COUNT+1))
